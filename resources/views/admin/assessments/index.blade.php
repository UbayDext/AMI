<x-app-layout>
    <div class="mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8"
        ...
        <div data-onboarding-assessment="intro" class="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
            ...
            <div class="flex gap-2">@if(auth()->user()->hasRole('admin'))<button type="button" onclick="window.dispatchEvent(new CustomEvent('restart-assessment-onboarding'))" class="h-11 rounded-xl border border-blue-200 px-4 text-sm font-bold text-blue-600 dark:border-blue-800 dark:text-blue-400">Lihat Panduan</button>@endif<a data-onboarding-assessment="create" href="{{ route('admin.assessments.create') }}" class="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5 hover:bg-blue-700">
        ...
        @endphp
        <div data-onboarding-assessment="stats" class="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-5">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
            <form data-onboarding-assessment="filters" method="GET" class="grid gap-3 border-b border-slate-200 p-4 dark:border-slate-700 sm:grid-cols-2 lg:grid-cols-[minmax(240px,1fr)_200px_200px_auto] lg:p-5">
            ...
            <div data-onboarding-assessment="table" class="hidden overflow-x-auto md:block">
    ...
    @if(auth()->user()->hasRole('admin'))
        @push('scripts')
            @include('admin.assessments.partials.onboarding')
        @endpush
    @endif
</x-app-layout>
