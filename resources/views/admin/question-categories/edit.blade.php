<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Question Category</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui kategori bank soal</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.question-categories.update', $category) }}">
                @csrf
                @method('PUT')
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100">Form Kategori</h2>
                        </div>
                    </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" required
                                    ...
                                    placeholder="Masukkan nama kategori (contoh. PBA, PGMI, dsb.)">
                            </div>
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Nama kategori harus unik dan representatif</p>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-5 bg-white dark:bg-gray-800 border-t border-dashed border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-category-deletion')"
                        ...
                        <div class="flex items-center gap-3 w-full sm:w-auto justify-end">
                            <a href="{{ route('admin.question-categories.index') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 text-sm font-medium transition-colors shadow-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <x-modal name="confirm-category-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="POST" action="{{ route('admin.question-categories.destroy', $category) }}" class="p-8 text-center relative">
            @csrf
            @method('DELETE')
            ...
            <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Hapus Kategori Ini?</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-7">
                Tindakan ini tidak bisa dibatalkan.<br>Sub-kategori mungkin akan kehilangan hirarkinya.
            </p>
