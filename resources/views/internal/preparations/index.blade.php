    ...
    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-5 space-y-4">
                {{-- Stage selector --}}
                <div class="bg-white dark:bg-gray-800 rounded shadow p-5">
                    ...
                    <div class="mt-3 w-full bg-gray-200 dark:bg-gray-700 rounded h-2 overflow-hidden">
                        <div class="h-2 bg-emerald-500" style="width: {{ $percent }}%"></div>
                    </div>
                    <div class="mt-2 text-xs text-gray-600 dark:text-gray-400">{{ $percent }}%</div>
                    ...
                    <div class="mt-4 space-y-2">
                        ...
                        @empty
                        <div class="text-sm text-gray-500 dark:text-gray-400">Belum ada tahap. Tambahkan data stage dulu.</div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded shadow p-5">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $active->description }}</div>
                            @endif
                        </div>
                        @if (session('success'))
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        <div class="border rounded p-4 {{ $t->is_done ? 'bg-emerald-50 border-emerald-200' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700' }}">
                            ...
                            <div class="mt-3">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Maks 10MB. PDF/Doc/Image boleh.</div>
                            </div>
                            ...
                            <div class="mt-3 space-y-2">
                                ...
                                @empty
                                <div class="text-sm text-gray-400 dark:text-gray-500">Belum ada file.</div>
                                @endforelse
                            </div>
                        </div>
                        @empty
                        <div class="text-sm text-gray-500 dark:text-gray-400">Tidak ada task untuk tahap ini.</div>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-7">
                <div class="bg-white dark:bg-gray-800 rounded shadow p-5 lg:sticky lg:top-6">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                    ...
                    <div class="mt-4">
                        @if(!$previewUrl)
                        <div class="p-6 border rounded bg-gray-50 dark:bg-gray-900/50 text-gray-600 dark:text-gray-400 text-sm">
                            Belum ada dokumen untuk dipreview. Upload file pada salah satu task.
                        </div>
                        @else
                        <div class="text-sm mb-3">
                            <div class="font-medium break-all">{{ $previewName }}</div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $previewMime }}</div>
                        </div>

                        <div class="border rounded overflow-hidden bg-white dark:bg-gray-800" style="height: 640px;">
                            @if($isPdf)
